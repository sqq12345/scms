...
class ReBcProduct{
    ...
    public function addProduct($data){
        ...
        $hotLevel = isset($data['hotLevel']) ? intval($data['hotLevel']) : 3; //商品热门级别
        ...
        if($ret){
            ...
            $logModel->ModelAdd($userid,'product',$ret,43);
    ...
    public function addSku($data){
        ...
        if(sizeof($list) > 0){
            ...
            foreach ($list as $val){
                ...
                $limitTime = isset($val['limitTime']) ? intval($val['limitTime']) : 1;
                $minPurchaseNum = isset($val['minPurchaseNum']) ? intval($val['minPurchaseNum']) : 1;
                $limitNum = isset($val['limitNum']) ? intval($val['limitNum']) : 999;
                $fareId = isset($val['fareId']) ? intval($val['fareId']) : 2;
                ...
                if(empty($model)|| $price == 0 || $dutyFreePrice == 0){
                    set_return_value(WILL_FIELD_NULL, '');
                    return false;
                }
                $fareTemplate = $fareTemplateModel->ModelInfo($fareId);
                if(empty($fareTemplate)){
                    set_return_value(WILL_FIELD_NULL, '');
                    return false;
                }
                $region = $fareTemplate['dispatchRegion'];
                $ret = $skuModel->BcModelAdd($pid,$modelName,$attr,$img,$ref,$stock,$weight,$price,$dutyFreePrice,$costPrice,$minPurchaseNum,$limitNum,$limitTime,$status,$userid,$detailImg,$fareId,$region,$vip_price, $tax, $express_free, $level, $commend, $sort, $sales);
                $logModel->ModelAdd($userid,'productSku',$ret,44);
            }
        }
        if($ret){
            set_return_value(RESULT_SUCCESS, $dataval);
    ...
    public function updateProduct($data){
        ...
        if($ret){
            $logModel = get_load_model('operationLog');
            $logModel->ModelAdd($userid,'product',$id,45);
            set_return_value(RESULT_SUCCESS, $dataval);
        }else{
            set_return_value(DEFEATED_ERROR, $dataval);
        }
    ...
    public function updateSku($data){
        ...
        if(sizeof($list) > 0){
            ...
            foreach ($list as $val){
                ...
                //销量
                $sales = isset($val['sales']) ? intval($val['sales']) : 100;
                if(empty($attr)|| $price == 0 || $dutyFreePrice == 0 || $costPrice == 0){
                    set_return_value(WILL_FIELD_NULL, '');
                    return false;
                }
                $fareTemplate = $fareTemplateModel->ModelInfo($fareId);
                ...
                $region = $fareTemplate['dispatchRegion'];
                if($id > 0){
                    $ret = $skuModel->BcModelUpdateSku($id,$modelName,$attr,$img,$ref,$stock,$weight,$price,$dutyFreePrice,$costPrice,$minPurchaseNum,$limitNum,$limitTime,$status,$userid,$detailImg,$fareId,$region,$vip_price, $tax, $express_free, $level, $commend, $sort, $sales);
                }else{
                    $ret = $skuModel->BcModelAdd($pid,$modelName,$attr,$img,$ref,$stock,$weight,$price,$dutyFreePrice,$costPrice,$minPurchaseNum,$limitNum,$limitTime,$status,$userid,$detailImg,$fareId,$region,$vip_price, $tax, $express_free, $level, $commend, $sort, $sales);
                }
            }
    ...
    public function updateStatus($data){
        ...
        if($ret){
            $logModel = get_load_model('operationLog');
            $logModel->ModelAdd($userid,'product',$id,46);
            set_return_value(RESULT_SUCCESS, $dataval);
        }else{
            set_return_value(DEFEATED_ERROR, $dataval);
        }
    ...
    public function getInfo($data){
        ...
        $fareTemplateModel = get_load_model('fareTemplate');
        $fareList = $fareTemplateModel->BcModelList();
        $dataval['fareList'] = $fareList;
        $dataval['model']['name'] = '';
    ...
    public function validateProductName($data){
        $dataval = array();
        $userid = isset($data['userid']) ? intval($data['userid']) : 0;
        $type = isset($data['type']) ? intval($data['type']) : 0;
        $id = isset($data['id']) ? intval($data['id']) : 0;
        ...
        $model = get_load_model('reProduct');
        $info = $model->BcModelInfoByName($name,$id);
        if(!empty($info)){
            set_return_value(PRODUCT_NAME_ERROR, $dataval);
        }else{
            set_return_value(RESULT_SUCCESS, $dataval);
        }
    }

    public function validateSkuRef($data){
        $dataval = array();
        $userid = isset($data['userid']) ? intval($data['userid']) : 0;
        $type = isset($data['type']) ? intval($data['type']) : 0;
        $id = isset($data['id']) ? intval($data['id']) : 0;
        $ref = inject_check(isset($data['ref']) ? $data['ref'] : '');   //ref
        if ($userid == 0 || $type < 3) {
            set_return_value(AUTH_ERROR, '');
            return false;
        }
        ...
        $info = $model->BcModelInfoByRefAndId($ref,$id);
        if(!empty($info)){
            set_return_value(REF_ERROR, $dataval);
        }else{
            set_return_value(RESULT_SUCCESS, $dataval);
        }
    }
